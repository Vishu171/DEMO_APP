class few_shot_settings:
    ...
    def get_examples():
            ...
            },
	    {
                ...
                "sql_cmd": '''SELECT A.QUARTER_NAME AS "QUARTER NAME", B.BU AS "BU", to_varchar(A.AMOUNT, '$ 999,999,999.90') AS "ON-HAND INVENTORY AMOUNT"
                            FROM FINANCIALS.MARVELL_DEMO.INVENTORY_ON_HANDS A LEFT JOIN FINANCIALS.MARVELL_DEMO.ITEM_DETAILS B ON B.ITEM_WID = A.ITEM_WID WHERE A.QUARTER_NAME = CASE WHEN MONTH(CURRENT_DATE()) IN (11, 12) THEN CONCAT(YEAR(CURRENT_DATE())+1, CONCAT('-Q', 4-1)) END;''',
            },
        ]
        return examples
